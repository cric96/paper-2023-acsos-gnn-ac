...
Over the years, several different neural networks have been proposed to solve specific tasks. 
%
Whereas Convolutional Neural Networks are designed to be used with image and spatial-like data (e.g., audio),
 Graph Neural Networks (GNNs) are a novel model used to process graph-structured data. 
%
...
Given $X_v$ a feature vector associated to each node $v \in V$, 
 ...
 In modern GNNs, the node embedding $X'_v$ is computed by aggregating information from the node's neighbors $N(v)$,
...
%
$\operatorname{AGGREGATE} $ instea is a function that aggregates the information from the neighbors of a node $v$
  and it could be a simple sum or a more complex function such as a neural network layer.
%
This formulation allows GNNs to effectively process and extract features from graph-structured data by iteratively aggregating and transforming information from the node's neighbors.

GNN are used in several fields such as social network analysis, chemistry, and physics.
In this paper, we use GNNs to learn a local behavior for each agent in a multi-agent system (more details in Section~\ref{sec:approach}).
