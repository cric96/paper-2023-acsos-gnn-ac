...
From this field, we construct the feature vector $f_v$ for each node $v$ in $G_t$ as follows:
 $f_v = (\theta^v_t, o^v_t)$. \lukas{we used $o_v$ to describe the observation of node $v$ in section II.C - so this would not include field information, right?}
...
\lukas{maybe: ``While the graph, containing the aggregate information, might appear as global knowledge, this is not the case as the information is never aggregated globally. The individual nodes only combine information from their local neighbourhood.''}
%
...
For the second point,
 ...
 It is known that \acp{GNN} have a certain ability to generalise to new structures and scale with different nodes~\cite{??}. 
% 
Additionally, 
...
% 
 During learning, it is as if we have a \emph{single} agent collectively choosing which action to take (i.e., resulting action graph). \lukas{this makes it sound like a singe agent has all information, selecting the actions for all other agents...}
%
\begin{algorithm}
  ...
  \While{not done}{
  Observe current graph observations $G_o$; \lukas{whats $G_o$?}
  
%  $r \leftarrow $ randomNumber();
%  
%	\If{$r \leq \epsilon$}{select a random graph action $G_a$}
%	\Else{select $G_a =$\\
...
%  
  ...
  Sample a batch of graph transitions $(G^i_o,G^i_a,G^i_r,G'^i_o)$ 
   from $\mathcal{D}$ and merge them in $(G^b_o,G^b_a,G^b_r,G'^b_o)$; \lukas{how does the merge work? do we explain this?}

  Compute the target Q-value for each node $v$ in the graph $G$: 
  $y = G^b_r + \gamma * \max{a'} Q(o'i,a';\theta^{-})$;
  
  Compute the current expected value for each node in the graph:
  $y^* = Q(s'i,a';\theta)$;
...
%% Put three subfigures of simulations, start, after, end
\begin{figure}[t]
  \centering
  ...
  the circle area on the top right represent the phenomenon to be monitored, 
  and the gray area represent the area covered by the robots.}
...
To test the effectiveness of the proposed approach, 
 ...
 \lukas{``Initially we consider a stationary phenomenon before expanding towards a moving phenomenon. Additionally, the phenomenon can change its shape during runtime. Finally, the phenomenon may have varying areas of interest, defined by an underlying distribution function. Figure~\ref{fig:simulations} illustrates a set of 25 agents and a single, stationary phenomenon with a with a 3D Gaussian distribution.''}
...
Additionally, each node has a coverage range $\omega$ 
 ...
 which in this case depends on a $\mathbb{O}$ range. \lukas{$\mathbb{O}$  not defined.}
...
\begin{enumerate}
\item Maximise the number of drones within the phenomena ($ndp$)
...
This will lead the system to prefer a configuration in which every node is present in the system. \lukas{do you mean `... is present within the phenomenon'?}
%
...
% they are not able to move in the environment due to the reduced observability of the system. 
 ...
 Finally, to maximise the coverage, 
  we decided to calculate the maximum and minimum distance of the neighbourhood and give more value in case these two values are similar. \lukas{minimum / maximum distance to what? the next neighbour? measuring the phenomena?}
%
...
\end{equation*}
where 
$d_{max}$ and $d_{min}$ describes the maximum and minimum distance of the node to \lukas{??}, respectively. This distance is normalized by the maximum communication range of each agent $\mathbb{O}$.
...
%% Subfigures of coverage-in-time and reward-in-time
