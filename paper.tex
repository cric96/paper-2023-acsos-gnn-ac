...
This article focuses on the definition of decentralized collective intelligence within the context of large-scale distributed systems. 
%
Specifically, we consider the case where agents can interact through 1-hop neighbourhood connections and have a local perception of the environment, 
 which is typical in systems such as swarm robotics or large-scale IoT. 

We model the system at a given time instance as a graph, $G = (V, E)$, 
...
Each node $v$ is associated with an observation (or feature set), $o_v$, 
 which can come from sensors installed on it and messages received from the neighbourhood in the previous step.

Our goal is to find a homogeneously distributed controller $\pi$ that, 
 starting from only local configurations, leads 
 the system to achieve a graph $G_T$ that satisfies certain collective requirements through cooperation, such as spatial area coverage, phenomena tracking, and robot aggregation.
...
%
Let $ G $ be the above-defined graph.
%
Given $o_v$ a feature vector associated with each node $v \in V$, 
 the goal of a GNN is to learn the node embedding $h_v$ for each node $v \in V$.
 In modern GNNs, the node embedding $h_v$ is computed by aggregating information from the node's neighbours $N(v)$,
