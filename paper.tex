...
%
Examples of CPSs include swarm robotics, smart cities, sensor networks, and social systems. 
...
A key question in designing swarm-lie systems is how to create distributed controllers for the agents that can enable them to perform complex collective tasks. 
...
Both approaches have advantages and disadvantages. 
 Manual design allows programmers to specify desired properties for the system declaratively. However, manual design can be tedious and error-prone.
%
...
In this paper, we propose a novel hybrid approach that combines manual design and automatic design of distributed controllers.
...
Specifically, we propose a solution called field-informed reinforcement learning (FIRL) that utilizes aggregate computing~\cite{Beal2015Computer} together with a \ac{GNN}~\cite{Zhou2020AIOpen} in combination with a reinforcement learning approach, namely \ac{DQN}~\cite{mnih2015human}. 
...
%
The main contribution of FIRL is that it allows us to create distributed controllers that are informed by a collective knowledge that has been distilled during learning, but that use only local information at runtime. This way, FIRL can achieve a balance between manual design and automatic design, combining the benefits of both approaches while mitigating their drawbacks.
%% Perhaps to much detail 
FIRL can also be seen as a way of bridging the gap between symbolic and sub-symbolic AI methods, by integrating declarative programming with deep learning.
...
Finally, the \emph{Moving} experiment emphasizes how the informed version generates a more robust policy for new scenarios. 
 Indeed, we observe that the our approach using field-informed reinforcement learning maintains higher coverage and a greater number of agents on the target phenomenon compared to the other two solutions.
%
...
\begin{figure*}
  ...
  \begin{subfigure}[b]{0.9\linewidth}
    ...
    \caption{Ratio of coverage of the phenomena in the three types of experiments. Our field-informed reinforcement learning approach can outperform other approaches lacking field information.}
