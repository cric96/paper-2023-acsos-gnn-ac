...
The proposed simulation consists 
 of a set of agents placed in a 2D space capable of perceiving the intensity 
 of the phenomenon of interest through an installed sensor $\zeta_v$ with $v \in \mathbb{V}$. 
 (e.g., camera, temperature sensor, etc.). 
%
Additionally, each agent has a coverage range $\omega$ 
...
% they are not able to move in the environment due to the reduced observability of the system. 
 ...
 Finally, to maximise the coverage, 
  ...
  \lukas{minimum / maximum distance to what? the next neighbour? measuring the phenomena?}
%
...
\end{equation*}
where 
$d_{max}$ and $d_{min}$ describes the maximum and minimum distance of the agent to \lukas{??}, respectively. 
This distance is normalized by the maximum communication range of each agent $\mathbb{O}$.
