...
Initially, 
  ...
  Finally, the phenomenon may have varying areas of interest, defined by an underlying distribution function. \lukas{are we utiliising this in any way? i.e. do the agents respond to intensity of their measurement. -- YES. It is part of the feature set. While we only use Guassian distributions but we can use any other distribution and shape. }
...
\subsection{Scenario}
Figure~\ref{fig:static} illustrates the scenario used during the training % \lukas{is this training the \ac{GNN}? }, 
...
\lukas{should stay here but this is the explicit implementation of the field:}
...
%
For each episode, 
 the 25 agents are semi-randomly \lukas{what is semi-random? are the position fixed but the agents random? or is it in a specific layout (e.g. lattice) but with some randomness of the actual position? lattice layout but random variation around a fixed point.} positioned on a grid without knowing the correct position of the phenomenon, 
 but that is fixed in the top right corner.  \lukas{so for all 20,000 experiences, the phenomena is exactly as shown in fig 2a during training? --> WE ONLY USE THE SINGLE PHENOMA IN A SINGLE FIXED LOCATION BUT WITH VARYING POSITIONS OF AGENTS!!}
...
Focusing now on the test phase, % in~\Cref{fig:test}
 we observe that the field-informed version achieves higher coverage than the other approaches in all scenarios since it shows the ability of our solution to generalize to situations--see \Cref{fig:simulations}. 
% 
...
In contrast, the uninformed version also maintains a fair division but with significantly different coverage between the two parts, 
 indicating an imbalance among the nodes in one of the zones.
%
...
The uninformed GNN version, however, 
 fails to generalize its movement behaviour, as evidenced by the simulations where the agents, once reaching the target zone, stop moving due to tracking issues.
