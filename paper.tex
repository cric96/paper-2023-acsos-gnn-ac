...
\begin{algorithm}
  \KwIn{Environment $\mathcal{E}$, graph replay buffer $\mathcal{D}$, target network $\theta^{-}$, current network $\theta$, exploration strategy $\epsilon$}
  ...
  Execute action $a$ in the environment $\mathcal{E}$ and observe the reward $r$ and the next state $s'$;

  Store transition $(s,a,r,s')$ in $\mathcal{D}$;

  Sample a batch of transitions $(s_i,a_i,r_i,s'i)$ from $\mathcal{D}$;

  Compute the target Q-value for each transition: $y_i = r_i + \gamma \max{a'} Q(s'i,a';\theta^{-})$;

  Update the current network weights using gradient descent: $\theta \leftarrow \theta - \alpha \nabla{\theta} \frac{1}{N} \sum_i (y_i - Q(s_i,a_i;\theta))^2$;
...
To test the effectiveness of the proposed approach, 
 ...
 specifically the tracking and coverage of a spatiotemporal phenomenon, cf. tracking a wildfire 
...
Additionally, each node has a coverage range $\omega$ 
 ...
 Each drone can only communicate directly with its own neighborhood $\mathcal{N}$, 
...
\item Minimize the number of drones without a neighbourhood
