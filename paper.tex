...
Given a time $t$, the system can be also represented as a graph $G_t = (V_t, E_t)$, 
  ...
  Each node is then decorated with the local observation $f_v = o_v$. 
...
%
Let $\Gamma$ be the aggregate program that takes a graph $G_t$ decorated by $o_v$ (i.e., the local observation)), 
...
From this field, we construct the feature vector $f_v$ for each node $v$ in $G_t$ as follows:
 $f_v = (\theta^v_t, o_t)$. \lukas{we used $o_v$ to describe the observation of node $v$ in section II.C - so this would not include field information, right?}
