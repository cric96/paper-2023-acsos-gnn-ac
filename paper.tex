...
impacted by the challenges posed by the very notion of distributed intelligence.  \lukas{what are these challenges?}
%
The problem is even more evident in large-scale collective adaptive systems, 
where agent interactions are necessarily proximity-based, thus making the emergence of controlled global collective behaviour harder. \lukas{what does it make harder?}
% 
...
In this paper, we consider a new \emph{hybrid} approach called  \emph{Field-Informed reinforcement learning}, 
  which uses manually designed \emph{computational fields} (globally distributed data structures) to manage the interaction between agents and a \emph{reinforcement learning technique} in combination with \emph{Graph Neural Networks} to learn the local behaviour necessary to solve collective tasks. 
%
