...
%
Let $\Gamma$ be the aggregate program that takes a graph $G_t$ decorated by $o^t_v$, 
...
From this field, we construct the feature vector $f_v$ for each node $v$ in $G_t$ as follows:
 $f_v = (\theta^v_t, o^t_v)$. \lukas{we used $o_v$ to describe the observation of node $v$ in section II.C - so this would not include field information, right?}
...
%  
  ...
  Sample a batch of graph transitions $(G^i_o,G^i_a,G^i_r,G^i_o)$ 
   from $\mathcal{D}$ and merge them in $(G_o,G_a,G_r,G_o)$; \lukas{how does the merge work? do we explain this?}
