...
\begin{figure}
  ...
  \caption{Simulation of the case study scenario in Alchemist. 
...
\subsection{Scenario}
Figure~\ref{fig:example} illustrates the scenario used during the learning \lukas{is this training the \ac{GNN}? }, 
which consists of a set of 25 agents and a single, stationary phenomenon with a 3D Gaussian 
...
%
Additionally, each agent has a coverage range $\omega$ (fixed to 7.5 meters) 
 that describes the area it can monitor. 
 Each agent can only communicate directly with its own neighbourhood $\mathcal{N}$, 
 which in this case depends on a $\mathbb{O}$ range fixed to 30 meters. %\lukas{$\mathbb{O}$  not defined.}
From the neighbourhood, 
 each agent can also perceive the direction of the other agents.
%
Each agent moves following a certain action composed of two components ($\alpha$, $\theta$) 
 which respectively describe the angle and intensity of the movement (i.e., the velocity vector). 
 \lukas{$\theta$ used for target weights...} 
 \lukas{not sure if this paragraph should be its own subsection. or at least it needs  different title.}
 For the evaluation, we explore three different types of scenarios. The first type considers only a single, stationary phenomenon. The second type considers multiple phenomena, requiring the agents to make a decision which phenomena to cover. Finally, the third type considers a single moving phenomena which the agents have to follow throughout the simulation. In all three types of scenarios, the shape and underlying distribution is initially unknown to the agents but remains constant for the entire duration of the simulation \lukas{correct?}. We generated random scenarios for each type of scenario. Examples of such types are shown in Figure~\ref{??} \lukas{can we create a figure with at least 3 sample scenarios (1 stationary, 2 split, 1 moving (with a line indicating its trajectory))?} We compare our field-informed reinforcement learning approach against a baseline approach utilising a \ac{MLP} as well as an approach only relying on \acp{GNN}. In all approaches, the underlying neural network (i.e., the \ac{MLP} and the \ac{GNN}) are trained with \lukas{xyz} samples of a single, stationary phenomena. \lukas{we should describe the training a bit.}
 \lukas{do we need more information on the MLP?}
